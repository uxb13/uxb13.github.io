...
'''
    available_cells = board_2048.get_available_cells(state)
    for i in available_cells:
        for j in [2,4]:
            copy_state = copy.deepcopy(state)
            board_2048.insert_new_item_minplay(copy_state,i,j)
            v = min(v,max_val_a_b(copy_state,depth-1,alpha,beta,params))
            if v<=alpha:
                return v
            beta = min(beta,v)
'''
